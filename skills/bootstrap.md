...
```markdown
### Available Skills
- [Set up project tracking](skills/linear/setup.md)
- [Create a database table](skills/supabase/setup.md)
- [Send a transactional email](skills/resend/setup.md)
```

### Step 4: Create Skills Directory

Create the skills directory structure for all selected services. For each selected service, create a `skills/<service>/setup.md` file with:
...
Use the existing service setup files as templates (see Phase 2 files in this repo). For services without a pre-written setup guide, create a minimal one with the structure above.

Also create cross-platform skills for any service pairs that have synergies:
- Linear + Notion → `skills/cross-platform/notion-sync-to-linear.md`
- Shopify + Xero → `skills/cross-platform/shopify-orders-to-xero.md`
